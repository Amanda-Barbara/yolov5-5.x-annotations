...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        self.balance = {3: [4.0, 1.0, 0.4]}.get(det.nl, [4.0, 1.0, 0.25, 0.06, .02])  # P3-P7 3个layer的输出权重分别是[4.0, 1.0, 0.4],增加小目标检测的损失权重
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                pwh = (ps[:, 2:4].sigmoid() * 2) ** 2 * anchors[i]  # 保证了预测框的宽高与锚框的宽高比最大不超过4，加速边框回归模型的收敛
    ...
    def build_targets(self, p, targets):
        ...
        for i in range(self.nl):  # self.nl: number of detection layers   Detect的个数 = 3
            ...
            if nt:  # 开始匹配  Matches
                ...
                # https://flyfish.blog.csdn.net/article/details/119276814
